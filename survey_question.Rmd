...
sam.n <- 10 #attempted sample size

store <- rep(NA, sims)
for (i in 1:sims){
  store[i] <- mean(pop[sample(1:200,sam.n),] %>%
                     filter(A==1) %>%
                     pull(satisfied))
}
#PH: in this for-loop, we're taking a random sample of visitors, subsetting out those that are "non-reporters" (A==0) and then taking the mean of their satisfaction. We do this "sims" number of times (1000).
...
design <- population + reporting + sampling + my_estimand
