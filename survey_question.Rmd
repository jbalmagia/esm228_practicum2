...
#Minnie: To make satisfaction a binary condition, a response of "neutral" or more negative is considered "dissatisfied" and both "somewhat satisfied" and "very satisfied" are considered "satisfied".
...
my_estimand <- declare_estimands(mean(pop$satisfied),
                                 label = "Ybar")
#From Patrick: Estimand is the mean level of knowledge among the sample.
```
...
donor_mean <- function(data){
  ...
  estimate = data %>% filter(A==1) %>% #PH: subset out units that are not sampled.
   # group_by(port) %>% 
    summarise(estimate=mean(donor, na.rm = TRUE))) #%>%
...
answer <- declare_estimator(
  handler = tidy_estimator(donor_mean), estimand = my_estimand)
answer(sam)
